...
export const validationSchema = yup.object().shape({
    ...
    password: yup
        .string()
        .matches(/(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*\W)/, 'Password must contain a number, an uppercase letter, a lowercase letter, and a special character.')
    ...
    picture: yup
        ...
        .test('fileType', 'Only png, jpeg, and jpg are allowed.', value => {
            return value && value.length > 0 && ['image/png', 'image/jpeg'].includes(value[0].type);
        })
