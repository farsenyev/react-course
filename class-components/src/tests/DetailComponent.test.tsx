...
import { DetailComponent } from '../app/components/DetailedComponent';
...
test('displays loading indicator while fetching data', async () => {
    fetchMock.mockResponseOnce(JSON.stringify({ name: 'Luke Skywalker', films: [] }));
...
test('displays films associated with the item', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Luke Skywalker')).toBeInTheDocument());
    await waitFor(() => expect(screen.getByText('Films: A New Hope')).toBeInTheDocument());
});
