...
jest.mock('@reduxjs/toolkit/query/react', () => {
    ...
    // Инициализируем базовый запрос с базовым URL.
    const baseQuery = originalModule.fetchBaseQuery({ baseUrl: 'https://swapi.dev/api/' });

    // Определение моковой функции запроса с типизацией для TypeScript
    const mockBaseQuery: BaseQueryFn<string | { url: string, method?: string }, unknown, unknown> = async (args) => {
...
describe('itemsApi tests', () => {
    ...
    it('successfully fetches items', async () => {
        const result = await store.dispatch(itemsApi.endpoints.getItems.initiate({ item: 'people', page: 1 }));
    ...
    it('fetches item details successfully', async () => {
        const result = await store.dispatch(itemsApi.endpoints.getDetails.initiate({ category: 'people', id: '1' }));
