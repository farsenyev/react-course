...
describe('SearchComponent', () => {
    ...
    test('handles navigation with empty search value', () => {
        const { getByText } = renderWithProviders(<SearchComponent onSearch={mockOnSearch} />);
        fireEvent.click(getByText('Next')); // Переход на следующую страницу
        expect(mockNavigate).toHaveBeenCalledWith('/testValue?page=2');
    });

    test('handles navigation with specific search value', () => {
        const { getByText, getByPlaceholderText } = renderWithProviders(<SearchComponent onSearch={mockOnSearch} />);
