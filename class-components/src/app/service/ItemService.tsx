...
export const itemsApi = createApi({
  ...
  endpoints: (build) => ({
    getItems: build.query<IPeopleAPI | IFilmsApi | IStarshipsApi, { item: string, page: number }>({
      query: ({ item, page }) => (page ? `/${item}/?page=${page}` : `/${item}`),
    }),
    getDetails: build.query<IPeople | IFilm | IStarships, { category: string, id: string }>({
