...
export const ResultsComponent: React.FC<Props> = ({ items, onSelectItem }) => {
  const storeItems = useSelector<RootState, (IPeople | IFilm | IStarships)[]>(
  ...
  const checkBoxHandler = (
    ...
    const isExisting = storeItems.find(
      (e: IPeople | IFilm | IStarships) =>
        ('name' in e ? e.name : e.title) === identifier,
    );
